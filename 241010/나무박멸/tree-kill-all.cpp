		...
		for (int j = 1; j <= n; ++j)
		{
			if (mapp[i][j] > 0)						//성장할 대상 나무
			{
				...
				for (int k = 0; k < 4; ++k)			//2. 성장 크기 찾기
				{
					...
					if (mapp[nxt_row][nxt_col] <= 0) continue;						// 벽 || 제초제 뿌려진 곳 || 빈칸 제외
		...
		for (int j = 1; j <= n; ++j)
		{
			if (mapp[i][j] > 0)						//번식할 대상 나무
			{
				...
				for (int k = 0; k < 4; ++k)			//2. 번식 가능 칸 찾기
				{
					...
					if (nxt_row < 1 || nxt_row > n || nxt_col < 1 || nxt_col > n) continue;
					if (mapp[nxt_row][nxt_col] != 0) continue;							// 벽 || 제초제 뿌려진 곳 || 나무 제외
	...
	for (int i = 1; i <= n; ++i)
	{
		for (int j = 1; j <= n; ++j)
		{
			...
			if (mapp[i][j] > 0)												////1. 제초제 뿌릴 후보 지정
			{
				...
				////2. k번만큼 대각선 4 방향으로 뻗어가면서 박멸 개수 구하기
				for (int l = 0; l < 4; ++l)						// 방향
				{
					for (int new_k = 1; new_k <= k; ++new_k)	// k만큼 특정 대각선 방향 이동
					{
						...
						//제한조건들
						if (nxt_row < 1 || nxt_row > n || nxt_col < 1 || nxt_col > n) break;

						//// 벽 || 나무가 아예 없는 칸 || 제초제 칸 -> 이후 대각선으론 제초제가 전파X
	...
	for (int i = 0; i < 4; ++i)						// 방향
	{
		for (int j = 1; j <= k; ++j)	// k만큼 특정 대각선 방향 이동
		{
			...
			if (mapp[nxt_row][nxt_col] <= 0)
			{
				...
				// 나무가 아예 없는 칸 -> 해당 방향 전파 끝 
				if (mapp[nxt_row][nxt_col] == 0)
				{
					mapp[nxt_row][nxt_col] = c;
					break;
				}
				//제초제 뿌려져 있는 칸 ->  새로 뿌려진 해로부터 다시 c년동안 제초제가 유지
				if (c <= mapp[nxt_row][nxt_col] && mapp[nxt_row][nxt_col] < 0)
				{
					mapp[nxt_row][nxt_col] = c;
					//continue;
					break;
				...
				// 벽 -> 해당 방향 전파 끝
				if (walls.find({ nxt_row , nxt_col }) != walls.end())
				{
					//mapp[nxt_row][nxt_col] = c;
					break;							
				}
